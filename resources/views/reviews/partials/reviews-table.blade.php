                ...
                <table class="table table-reviews tablesaw tablesaw-stack table-custom" data-tablesaw-mode="stack">
                    <thead>
                        <tr>
                            ...
                            <th><a href="">Rating <i class="fa" aria-hidden="true"></i></a></th>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ route('view-user', auth()->user()->id) }}" target="_blank">{{ $r->display_name }}</a></td>
                        <td><input disabled name="rating" value="{{ $r->rating }}" type="number" class="rating" min=0 max=5 step=0.5 data-size="xs" ></td>
                        <td><a href="{{ preg_replace('/^(?!https?:\/\/)/', 'http://', $r->url) }}" target="_blank">{{ $r->url }}</a></td>
                        <td><a href="#" class="review-screenshot-thumb" data-screenshoturl="{{ url($r->screenshot) }}"><img width="30" src="{{ url($r->screenshot) }}" /></a></td>
                    </tr>
                    <tr class="tr-spacer"><td colspan=5></td></tr>
                    @endforeach
                    @if (!count($reviews))<tr><td colspan="5">No reviews found.</td></tr>@endif
                </table>      
                <div class="col-md-12 text-right"><a href="{{ route('create-reviews') }}">Create a Review</a></div>
